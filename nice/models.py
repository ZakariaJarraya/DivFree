    ...
    * a diagonal scaling matrix output layer
    ...
    def forward(self, xs):
        ...
        * ys: float tensor of shape (B,dim).
        """
        ys = self.layer1(xs)
        ys=self.Identity1(ys)
        ys = self.layer2(ys)
        ys=self.Identity2(ys)
        ys = self.layer3(ys)
        ys=self.Identity3(ys)
        ys = self.layer4(ys)
        ys=self.Identity4(ys)
        ys = torch.matmul(ys, torch.diag(torch.exp(self.scaling_diag)))
        ys=self.Identity5(ys)
        logits=self.finalLayer(ys)
