...
def finalstate ( reseau, data, lyapunov ):
    #relu=nn.ReLU()
    activation = {}
    ...
    distance1=torch.sum(torch.pow(activation['Identity']-lyapunov.c1, 2),dim=-1)
    distance2=torch.sum(torch.pow(activation['Identity']-lyapunov.c2, 2),dim=-1)
